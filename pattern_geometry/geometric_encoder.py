...
class GeometricEncoder:
    ...
    def encode_to_binary(self, token: str) -> str:
        ...
        if len(vertex_bits) != self.vertex_width:
            raise ValueError(
                f"Vertex bits must be {self.vertex_width} wide, got {len(vertex_bits)}"
            )
        ...
        if symbol not in self.SYMBOL_MAP:
            raise ValueError(
                f"Unknown symbol '{symbol}'. Valid symbols: {list(self.SYMBOL_MAP.keys())}"
            )
    ...
    def decode_from_binary(self, binary_string: str) -> str:
        ...
        if (len(binary_string) >= self.vertex_width + 4
                and binary_string[operator_start:operator_start + 2] == '11'):
            # Nested operator ||
            operator = '||'
