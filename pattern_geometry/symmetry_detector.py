"""
SymmetryDetector: Detects exploitable symmetries in source configurations.

Ported from Geometric-to-Binary-Computational-Bridge/Engine/symmetry_detector.py
...
class SymmetryDetector:
    """Detects reflective and rotational symmetry in point configurations."""

    def __init__(
        self,
        position_tol: float = 1e-4,
        magnitude_tol: float = 1e-4,
    ):
    ...
    def findSymmetries(
        ...
        Returns:
            list of symmetry dicts, each with:
                ...
                'plane' or 'axis': symmetry element description
        ...
        # Check rotational symmetries (2-fold, 3-fold, 4-fold, 6-fold) around each axis
        for axis_idx, axis_name in enumerate(["x", "y", "z"]):
            axis_vec = np.zeros(3)
            axis_vec[axis_idx] = 1.0
            for n_fold in [2, 3, 4, 6]:
                if self._check_rotation(
                    positions, strengths, types, centroid, axis_vec, n_fold
                ):
                    symmetries.append({
                        "type": "rotational",
                        "axis": axis_name,
    ...
    def _check_reflection(
        ...
        return self._configurations_match(
            positions, strengths, types, reflected, strengths, types
        )
    ...
    def _check_rotation(
        ...
        return self._configurations_match(
            positions, strengths, types, rotated, strengths, types
        )
        ...
        for i in range(n):
            ...
            for j in range(n):
                ...
                if (dist < self.position_tol
                        and mag_rel_diff < self.magnitude_tol
                        and types_a[i] == types_b[j]):
                    matched[j] = True
                    found = True
                    break
            if not found:
    ...
    def get_reduction_factor(self, symmetries: list[dict]) -> float:
